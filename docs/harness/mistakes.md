...
아래 패턴이 발생하면 실수로 간주하고 이 문서에 기록한다.

- 에이전트가 코드를 수정하거나 구현 방향을 제안한 뒤, 사용자가 "왜 확인 안 했냐", "이건 빠진 거 아니냐", "왜 그렇게 했냐"처럼 누락이나 판단 오류를 지적한 경우
- 에이전트가 "제가 잘못 생각했습니다", "그 부분은 놓쳤습니다", "맞습니다, 빠져 있습니다", "확인하지 못했습니다"처럼 자신의 누락, 오판, 검증 부족을 인정한 경우
- 코드 자체는 동작하더라도 사용자가 기대한 기존 프로젝트 컨벤션, 문서화 수준, 커밋 방식, 검증 절차를 빠뜨린 경우
- 사용자가 직접 수정 기준이나 운영 규칙을 보강해 달라고 요청한 이유가 이전 에이전트 응답의 누락 때문인 경우
