...
- 변경사항을 목적별로 분류하고, 관계없는 변경이 섞여 있으면 여러 커밋으로 나눌 계획을 세운다.
- `docs/harness/documentation-sync.md`의 커밋 전 체크리스트에 따라 코드 변경과 문서 변경이 함께 최신화되었는지 확인한다.
...
- API, 도메인, DB, 공통 응답/예외, 패키지, 실행 설정 변경이 있으면 관련 `docs/` 문서도 함께 갱신한다.
...
- 목적이 같은 관련 파일만 `git add`로 스테이징한다.
- 관계없는 변경사항이 남아 있으면 다음 목적의 커밋으로 반복한다.
- 같은 파일 안에 여러 목적의 변경이 섞여 분리하기 어려운 경우에만 예외적으로 한 커밋에 포함한다.
- `git commit`으로 커밋 실행
